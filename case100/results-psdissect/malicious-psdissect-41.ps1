function kEYLO`g`GeR {
    ...
    Set-`co`NtenT -Path $logPath -Value '' -Encoding U`TF8
    w`RiT`e-oUTp`UT '[safe] Simulated keystrokes written to '
    ...
    if ($True) {
        ...
        Wr`i`TE-`OutPut '[safe] Exfiltration simulated via .'
    }

    Wr`Ite-oUTp`UT '[safe] Magic stop string remains configured as '''' from '
}
