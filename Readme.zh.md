...
| `-OptimizationProfile` | `Default` | 行为调优 profile：`Default` / `Cmdline` / `AdaptiveCoverage` |
...
`OptimizationProfile` 用来做行为调优：

- `Default`：标准模式。
- `Cmdline`：更偏向命令行式样本，并对部分替换更保守。
- `AdaptiveCoverage`：优先在预算内完成，同时保留一次更有针对性的后续恢复机会，用于在轻量剥离后继续回收高价值结果。
