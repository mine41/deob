function KE`YLoGg`ER {
    ...
    Set-Content -Path $logPath -Value '' -Encoding u`Tf8
    Write-Output '[safe] Simulated keystrokes written to '
    ...
    if ($True) {
        ...
        Write-Output '[safe] Exfiltration simulated via .'
    }

    Write-Output '[safe] Magic stop string remains configured as '''' from '
}
